...
public class Player {
	...
	private Move produceRandomMove(Board board){
		...
		int randomRow = ThreadLocalRandom.current().nextInt(1, board.getNumberOfRows() + 1);
		int randomLeft = ThreadLocalRandom.current().nextInt(1, board.getRowLength(randomRow) + 1);
		while(!board.isStickUnmarked(randomRow, randomLeft)){
			randomRow = ThreadLocalRandom.current().nextInt(1, board.getNumberOfRows() + 1);
			randomLeft = ThreadLocalRandom.current().nextInt(1, board.getRowLength(randomRow) + 1);
		}

		int randomRight = ThreadLocalRandom.current().nextInt(randomLeft, board.getRowLength(randomRow) + 1);
		for(int i = randomLeft; i <= randomRight; i++){
			if(!board.isStickUnmarked(randomRow, i))
				randomRight = ThreadLocalRandom.current().nextInt(randomLeft, i);
		}
		return new Move(randomRow, randomLeft, randomRight);
